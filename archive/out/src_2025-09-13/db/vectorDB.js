...
export class VectorDB {
    ...
    async addDocument(key, text, metadata = {}) {
        try {
            ...
            await this.addEmbedding(key, vector, { ...metadata, text });
        }
        catch (error) {
            console.error('Failed to add document:', error);
            // Fallback: use simple hash-based vector
            const fallbackVector = this.generateFallbackVector(text);
            await this.addEmbedding(key, fallbackVector, { ...metadata, text });
        }
    }
    ...
    async save() {
        try {
            const dataPath = path.join(this.storagePath, 'vectors.json');
            fs.writeFileSync(dataPath, JSON.stringify(this.items, null, 2), 'utf8');
        }
        catch (error) {
