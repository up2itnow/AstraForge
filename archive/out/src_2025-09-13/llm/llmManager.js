...
export class LLMManager {
    constructor(testPanel) {
        this.panel = [];
        if (testPanel) {
            // Use provided test panel
            this.panel = testPanel;
        }
        else if (process.env.NODE_ENV === 'test' && process.env.OPENROUTER_API_KEY) {
            // In test environment, create panel from environment variables
            const models = process.env.OPENROUTER_MODELS_TO_USE?.split(',').map(model => model.trim()) || [];
            this.panel = models.map((model, index) => ({
                provider: 'OpenRouter',
    ...
    async generateResponse(provider, prompt) {
        // Find the first LLM of the specified provider or use the first available
        const config = this.panel.find(llm => llm.provider.toLowerCase() === provider.toLowerCase()) || this.panel[0];
        if (!config) {
            throw new Error(`No LLM configured for provider: ${provider}`);
        }
        const index = this.panel.indexOf(config);
        return await this.queryLLM(index, prompt);
    }
