...
describe('LLMManager', () => {
    ...
    describe('queryLLM', () => {
        ...
        it('should query OpenAI successfully', async () => {
            ...
            expect(mockAxios.post).toHaveBeenCalledWith('https://api.openai.com/v1/chat/completions', expect.objectContaining({
                model: 'gpt-4',
    ...
    describe('String similarity calculation', () => {
        it('should calculate similarity correctly', () => {
            ...
            const similarity = llmManager.calculateSimilarity('hello', 'helo');
            expect(similarity).toBeGreaterThan(0.7);
            expect(similarity).toBeLessThan(1);
        });
        it('should return 1 for identical strings', () => {
            const similarity = llmManager.calculateSimilarity('test', 'test');
            expect(similarity).toBe(1);
        });
        it('should handle empty strings', () => {
            const similarity = llmManager.calculateSimilarity('', '');
            expect(similarity).toBe(1);
        });
    });
    describe('Levenshtein distance', () => {
        it('should calculate edit distance correctly', () => {
            const distance = llmManager.levenshteinDistance('kitten', 'sitting');
            expect(distance).toBe(3);
        });
        it('should return 0 for identical strings', () => {
            const distance = llmManager.levenshteinDistance('same', 'same');
            expect(distance).toBe(0);
        });
        it('should handle empty strings', () => {
            const distance = llmManager.levenshteinDistance('', 'test');
