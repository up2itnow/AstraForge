...
`;
    ...
    async generateImplementationTasks(plan) {
        const prompt = `
    ...
    async generateIntegrationTasks(plan) {
        const prompt = `
    ...
    async generatePolishTasks(plan) {
        const prompt = `
    ...
    async generateDependencies(tasks, _plan) {
        const prompt = `
    ...
    generateBasicDependencies(tasks) {
        const dependencies = [];
        ...
        implementationTasks.forEach(task => {
            const relatedTestTask = testTasks.find(t => t.filePath.includes(task.filePath.split('/').pop()?.split('.')[0] || ''));
            if (relatedTestTask) {
                dependencies.push({
                    taskId: task.id,
    ...
    findIndependentTasks(tasks, dependencies) {
        ...
        fileMap.forEach(fileTasks => {
            if (fileTasks.length === 1) {
                ...
                const hasPhaseDependency = dependencies.some(dep => dep.taskId === task.id &&
    ...
    assembleTaskList(data) {
        ...
        const setupSection = this.formatTaskSection(data.tasks.filter((t) => t.type === 'setup'));
        const testSection = this.formatTaskSection(data.tasks.filter((t) => t.type === 'test'));
        const implementationSection = this.formatTaskSection(data.tasks.filter((t) => t.type === 'implementation'));
        const integrationSection = this.formatTaskSection(data.tasks.filter((t) => t.type === 'integration'));
        const polishSection = this.formatTaskSection(data.tasks.filter((t) => t.type === 'polish'));
    ...
    formatValidationChecklist(tasks) {
...
- [ ] No task modifies same file as another [P] task`;
    ...
    detectDependencyCycles(dependencies) {
        const graph = new Map();
        const cycles = [];
