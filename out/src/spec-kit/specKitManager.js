...
export class SpecKitManager {
    ...
    async generateTasks(workflowId) {
        ...
        if (!validation.valid) {
            const proceed = await vscode.window.showWarningMessage(`Task validation found issues:\n${validation.issues.join('\n')}\n\nProceed anyway?`, 'Proceed', 'Fix Issues');
            if (proceed !== 'Proceed') {
                return;
            }
        }
    ...
    async refineSpecification(workflowId, refinements) {
        const workflow = this.workflows.get(workflowId);
        if (!workflow || !workflow.spec) {
            throw new Error('Workflow or specification not found');
        }
    ...
    async createDirectoryStructure(dirs) {
        for (const dir of dirs) {
            await fs.promises.mkdir(dir, { recursive: true });
        }
    }
    async copySpecKitTemplates(templatesDir) {
...
`;
    ...
    async copySpecKitScripts(scriptsDir) {
    ...
    async createAstraForgeConstitution(memoryDir) {
...
`;
    ...
    formatResearchResults(researchTasks) {
    ...
    async showSpecificationResults(workflow) {
        ...
        const action = await vscode.window.showInformationMessage(message, 'Create Plan', 'Refine Spec', 'View Spec');
        switch (action) {
            case 'Create Plan':
    ...
    async showPlanResults(workflow) {
        ...
        const action = await vscode.window.showInformationMessage(message, 'Generate Tasks', 'View Plan', 'Review Research');
        switch (action) {
            case 'Generate Tasks':
    ...
    async showTaskResults(workflow) {
        ...
        const action = await vscode.window.showInformationMessage(message, 'Start Implementation', 'View Tasks', 'Export Tasks');
        switch (action) {
...
//# sourceMappingURL=specKitManager.js.map