...
`;
  ...
  private findIndependentTasks(tasks: Task[], dependencies: TaskDependency[]): Task[] {
    ...
    fileMap.forEach(fileTasks => {
      if (fileTasks.length === 1) {
        ...
        const hasPhaseDependency = dependencies.some(
          dep =>
            dep.taskId === task.id &&
  ...
  private assembleTaskList(data: any): string {
    ...
    const implementationSection = this.formatTaskSection(
      data.tasks.filter((t: Task) => t.type === 'implementation')
    );
    const integrationSection = this.formatTaskSection(
      data.tasks.filter((t: Task) => t.type === 'integration')
    );
    const polishSection = this.formatTaskSection(
      data.tasks.filter((t: Task) => t.type === 'polish')
    );
  ...
  private formatTaskSection(tasks: Task[]): string {
    return tasks
      .map(task => {
        ...
        const prerequisites =
          task.prerequisites.length > 0 ? ` (after ${task.prerequisites.join(', ')})` : '';
  ...
  private formatValidationChecklist(tasks: Task[]): string {
    const contractTests = tasks.filter(
      t => t.type === 'test' && t.description.includes('contract')
    );
    const entities = tasks.filter(
      t => t.type === 'implementation' && t.description.includes('model')
    );
...
- [ ] No task modifies same file as another [P] task`;
  ...
  public async validateTasks(taskList: TaskList): Promise<{ valid: boolean; issues: string[] }> {
    ...
    fileGroups.forEach((tasks, file) => {
      if (tasks.length > 1) {
        issues.push(
          `File conflict: ${file} modified by parallel tasks ${tasks.map(t => t.id).join(', ')}`
        );
      }
    });
